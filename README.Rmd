...
> See the [actual blog post](https://www.andrewheiss.com/blog/2019/02/16/algebra-calculus-r-yacas/).
...
That's `r n_pizza` pizzas or `r n_yogurt` yogurts. We can use this information to build a formula. Since we can draw a line using the $y = mx + b$ form, we need to figure out the slope ($m$) and y-intercept ($b$). The y-intercept here is `r n_yogurt`, or the number of yogurts he'd eat if x is 0, and the slope is the number of yogurts divided by the number of pizzas, or `r -n_yogurt / n_pizza`. That means the budget line is:
...
He gets `r utility_u(5, 5)` happiness points from 5 slices of pizza and 5 bowls of frozen yogurt. Neat.
...
```{r plot-indifference, warning=FALSE}
ggplot() + 
  # Budget line
  stat_function(data = tibble(x = 0:15), aes(x = x),
                fun = budget, color = "#638ccc", size = 1.5) +
...
$$
\partial u / \partial x = `r TeXForm(mu_x)`
$$
...
$$
\partial u / \partial y = `r TeXForm(mu_y)`
$$
...
$$
\frac{\partial u / \partial x}{\partial u / \partial y} = \frac{`r TeXForm(mu_x)`}{`r TeXForm(mu_y)`} = `r TeXForm(Simplify(mu_x / mu_y))`
$$
...
$$
`r TeXForm(Solve(paste(Simplify(mu_x / mu_y), "==", price_x, "/", price_y), y))`
$$
...
There! We have an answer. The optimal level of consumption is `r optimal_x` slices of pizza and `r optimal_y` bowls of frozen yogurt, which yields `r max_utility` utils of happiness.
...
```{r plot-everything, warning=FALSE}
ggplot() + 
  # Budget line
  stat_function(data = tibble(x = 0:15), aes(x = x),
                fun = budget, color = "#638ccc", size = 1.5) +
...
```{r complete-example, warning=FALSE, message=FALSE}
library(tidyverse)
library(Ryacas)
